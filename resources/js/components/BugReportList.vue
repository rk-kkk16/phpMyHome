    ...
    <tbody>
        ...
        <tr>
            <td :class="`star_lv${row.level}`" style="width:5.5em">
                ...
                <span
                    v-bind:class="{ 'hidden':!(row.state == 'done' && row.done_user_id)}"
                    class="profimg prfmini"
                    v-bind:style="`background-image:url(/users/icon/${row.done_user_id})`">
                </span>
            </td>
            <td style="width:6em;">
                {{levels[row.level]}}
        ...
        <tr v-bind:class="{'hidden':!(row.open==true)}">
            <td colspan="4">
                <div class="td-inner-left">
                    <p v-html="$options.filters.nl2br($options.filters.url2Link(row.description))"></p>
                </div>

                <div class="td-inner-right">
                    <button
                        v-bind:onclick="`if ($('#edit_${row.id}').css('display')=='none') { $('#edit_${row.id}').show(); }`"
        ...
        <!-- editform -->
        <tr v-bind:id="`edit_${row.id}`" style="display:none"><td colspan="4">
            <p>○ 編集</p>
            <table class="table table-bordered table-striped"><tbody>
                <tr>
                    ...
                    <td colspan="2" class="form-group">
                        <input type="text" class="form-group" v-bind:id="`edit_subject_${row.id}`" v-bind:value="row.subject">
                        <div v-bind:id="`mw_edit_subject_err_${row.id}`"></div>
                    </td>
                </tr><tr>
                    <th style="width:5em">分類</th>
                    <td class="form-group">
                        <select v-bind:id="`edit_level_${row.id}`" class="form-group">
                            <option value="5"
                                v-bind:selected="row.level==5"
                            >不具合</option>
                            <option value="3"
                                v-bind:selected="row.level==3"
                            >変更</option>
                            <option value="1"
                                v-bind:selected="row.level==1"
                            >機能追加</option>
                        </select>
                    </td>
                </tr><tr>
                    ...
                    <td colspan="2" class="form-group">
                        <textarea class="form-control" v-bind:id="`edit_description_${row.id}`" style="height:10em">{{row.description}}</textarea>
                        <div v-bind:id="`mw_edit_description_err_${row.id}`"></div>
                    </td>
                </tr>
            </tbody></table>

            <p style="width:100%;text-align:center">
                <button class="btn btn-primary" v-bind:onclick="`bugReportEditSave(${row.id});`">　O K　</button>
                <button class="btn btn-secondary" v-bind:onclick="`$('#edit_${row.id}').hide();`">Cancel</button>
            </p>
...
export default {
    ...
    methods: {
        loadNext() {
            ...
            let url = '/api/bugreport?page=' + self.page + '&' + paramstr;
        ...
        toggleState(id, nowstate) {
            ...
            let url = '/api/bugreport/toggle/' + id + '/' + newstate;
            axios.put(url, {}).then(function(response) {
                ...
                for (var i = 0; i < self.rows.length; i++) {
                    if (self.rows[i].id == response.data.data.id) {
                        ...
                        self.rows[i].done_user_id = response.data.data.done_user_id;
                        break;
                    }
                }
            })
            ...
            if (typeof(this.index[item.id]) != 'undefined') {
                for (var i = 0; i < this.rows.length; i++) {
                    if (this.rows[i].id == item.id) {
                        this.rows[i].subject = item.subject;
                        this.rows[i].description = item.description;
